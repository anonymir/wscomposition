...
namespace PFE_COMPOSITION
{
    ...
    public class ServiceCompositionP : System.Web.Services.WebService
    {
        ...
        private Composition GetBestNode(List<Composition> list)
        {
            ...
            {
                if (Distance(c.parameters, requete.output) < dMin)
                {
                    best = c;
                }
            }
        ...
        private void initEspaceRecherche(int t)
        {
            ...
            results.timeGeneration = (DateTime.Now - start).TotalMilliseconds;

            start = DateTime.Now;

            matrice = GetMatriceDistances(corpus, g, t);
